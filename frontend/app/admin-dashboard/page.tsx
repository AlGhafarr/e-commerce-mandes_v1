...
import { Filter, Star, ShoppingCart, ChevronLeft, ChevronRight, RotateCcw, Search, X, ArrowUpDown, MessageCircle, ArrowUp, ArrowLeft } from 'lucide-react';
...
export default function ProductPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FCF9EA]">
      ...
      <div className={`lg:hidden transition-all duration-300 ${isScrolled ? 'pt-20' : 'pt-28'}`}></div>

      <main className="container mx-auto px-4 pb-20 lg:py-8">
        ...
        <Link href="/">
          <div className="bg-white rounded-xl p-4 md:p-6 shadow-sm mb-6 border-l-4 border-[#F87B1B] mt-2 lg:mt-0 cursor-pointer hover:shadow-md transition">
            <div>
              ...
              <p className="text-gray-500 text-xs md:text-sm mt-1">{processedProducts.length} produk ditemukan</p>
            </div>
          </div>
        </Link>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="lg:w-1/4 hidden lg:block h-fit sticky top-24"><div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">{FilterContent}</div></aside>
      ...
      <button onClick={scrollToTop} className={`fixed bottom-6 right-6 bg-[#F87B1B] text-white p-3 rounded-full shadow-lg transition-all duration-300 z-50 ${showScrollTop ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}><ArrowUp size={24}/></button>
    </div>
  );
}